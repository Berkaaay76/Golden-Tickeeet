...
util.AddNetworkString("Killlogs")
...
net.Receive("Zufallszahl_user", function(len, plyr)
    ...
    if sb == 1 then
        ...
        if (table.Count(team.GetPlayers(TEAM_USA))) > (table.Count(team.GetPlayers(TEAM_RUS))) then
    ...
    elseif sb == 2 then
        timer.Simple(0.5, function()
            ...
            if (table.Count(team.GetPlayers(TEAM_RUS))) > (table.Count(team.GetPlayers(TEAM_USA))) then
                plyr:changeTeam(TEAM_USA, true)
...
hook.Add("PlayerSpawn", "WennSpeielerspawnt", function(playr)
    if DieTeams[playr:Team()] then
        ...
        timer.Simple(11.1, function() -- leider cooldown bis nächstes Spiel
            playr:Freeze(false)
        end)
end)
...
hook.Add("PlayerDeath", "WennSpielerstirbt", function(victim, inflictor, attacker)
    ...
    if counter_usa == (table.Count(team.GetPlayers(TEAM_RUS))) then
        for k, v in pairs(player.GetAll()) do
    ...
    if counter_rus == (table.Count(team.GetPlayers(TEAM_USA))) then
        for k, v in pairs(player.GetAll()) do
...
hook.Add("PlayerSay", "WennSpielersagt", function(sender, text)
    ...
    if text == "!killlogs" and sender:IsSuperAdmin() then
        net.Start("Killlogs")
        net.Send(sender)
...
net.Receive("waffengeben", function(len, plyr)
    ...
    Timestamp = os.time()
    TimeString = os.date("[%d.%m.%Y - %H:%M:%S]", Timestamp)
    ...
    if l == 1 then
        plyr:SetRunSpeed(100)
        plyr:SetWalkSpeed(100)
    end
    ...
    file.Append("degahtest/klassen.txt", "\n" .. TimeString .. " " .. spieler .. " ist in der Klasse: " .. waffenklasse.name)
    net.Start("farbenimlog")
    net.WriteString(spieler)
