...
net.Receive("Deathdaten", function()
    local op = net.ReadString()
    local ki = net.ReadString()
    local wp = net.ReadString()
    local ti = net.ReadString()

    table.insert(death_logs, {
        opfer = op,
        killer = ki,
        Waffe = wp,
        Zeit = ti
...
function meinelogs.Open()
    local Bildschirmbreite = ScrW()
    local Bildschirmhoehe = ScrH()
    meinelogs.Oberflaeche = vgui.Create("DFrame")
    meinelogs.Oberflaeche:SetSize(Bildschirmbreite * 0.4, Bildschirmhoehe * 0.5)
    meinelogs.Oberflaeche:MakePopup()
    meinelogs.Oberflaeche:Center()
    meinelogs.Oberflaeche:SetTitle("")
    meinelogs.Oberflaeche:DockMargin(20, 20, 20, 20)
    meinelogs.Oberflaeche:SetIcon("materials/Bilder/los.png")
    meinelogs.Oberflaeche:SetDraggable(false)

    meinelogs.Oberflaeche.Paint = function(me, w, h)
    ...
    local logs_deaths = vgui.Create("DListView", meinelogs.Oberflaeche)
    logs_deaths:Dock(FILL)
    logs_deaths:SetSize(Bildschirmbreite * 0.2, Bildschirmhoehe * 0.2)
    logs_deaths:AddColumn("Zeit"):SetFixedWidth(130) --Tabellenbreite der Zeit kleiner gemacht
    logs_deaths:AddColumn("Aktion")

    function logs_deaths:DoDoubleClick(lineID, line)
        SetClipboardText(line:GetColumnText(1) .. line:GetColumnText(2))
    end

    for k, deathlogs in pairs(death_logs) do
        local message = deathlogs.opfer .. " wurde mit " .. deathlogs.Waffe .. " getötet von: " .. deathlogs.killer
        logs_deaths:AddLine(deathlogs.Zeit, message) -- Table wird nach jedem Serverrestart resetet
    end

    logs_deaths.Paint = function(me, w, h)
        surface.SetDrawColor(Klassenauswahl.Farben["scrollpanels"]) --Kündigt quasi ein rechteck an
        surface.DrawRect(0, 0, w, h) -- Rechteck gefüllt
    end
end
...
net.Receive("Countdown_zuUser", function(len)
    ...
    timer.Simple(11.1, function()
        net.Start("Countdown_server")
        net.WriteInt(countdown, 3)
        net.SendToServer()
    end)
end)
...
net.Receive("allelogs", function()
    local fs = file.Open("degahtest/klassen.txt", "r", "DATA")
    chat.AddText(fs:Read(fs:Size()))
end)